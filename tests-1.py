...
def plot_scenarios(results: list[tuple[str, "CellFreeSimulationResults"]]):
    """
    Plot SINR and achievable-rate CDFs.

    Dotted line: Statistical CSI
    Solid line: Instantaneous CSI
    """

    fig_sinr, ax_sinr = plt.subplots(figsize=(7, 5))
    ...
    ax_rate.set(
        title="Per-user Achievable Rate CDF",
        xlabel="Rate [Mbits/s]",
        ylabel="CDF",
    )
    ax_rate.grid(True)
    ...
    fig_sinr.tight_layout()
    fig_rate.tight_layout()

    return fig_sinr, fig_rate
